...
var CpuLimitSeldonTemplate = `sort_desc(sum by (label_seldon_app,namespace) ((sum_over_time(kube_pod_labels{label_app_kubernetes_io_managed_by=~"seldon-core"}[{{.Range}}] {{.OffsetExp}}) / scalar(max(sum_over_time(kube_pod_labels[{{.Range}}] {{.OffsetExp}})))) * on(pod,namespace) group_right(label_seldon_app) sum by (namespace,pod,container) (kube_pod_container_resource_limits_cpu_cores )))`
var CpuRequestSeldonTemplate = `sort_desc(sum by (label_seldon_app,namespace) ((sum_over_time(kube_pod_labels{label_app_kubernetes_io_managed_by=~"seldon-core"}[{{.Range}}] {{.OffsetExp}}) / scalar(max(sum_over_time(kube_pod_labels[{{.Range}}] {{.OffsetExp}})))) * on(pod,namespace) group_right(label_seldon_app) sum by (namespace,pod,container) (kube_pod_container_resource_requests_cpu_cores )))`
var MemLimitSeldonTemplate = `sort_desc(sum by (label_seldon_app,namespace) ((sum_over_time(kube_pod_labels{label_app_kubernetes_io_managed_by=~"seldon-core"}[{{.Range}}] {{.OffsetExp}}) / scalar(max(sum_over_time(kube_pod_labels[{{.Range}}] {{.OffsetExp}})))) * on(pod,namespace) group_right(label_seldon_app) sum by (namespace,pod,container) (kube_pod_container_resource_limits_memory_bytes )))`
var MemRequestSeldonTemplate = `sort_desc(sum by (label_seldon_app,namespace) ((sum_over_time(kube_pod_labels{label_app_kubernetes_io_managed_by=~"seldon-core"}[{{.Range}}] {{.OffsetExp}}) / scalar(max(sum_over_time(kube_pod_labels[{{.Range}}] {{.OffsetExp}})))) * on(pod,namespace) group_right(label_seldon_app) sum by (namespace,pod,container) (kube_pod_container_resource_requests_memory_bytes )))`
